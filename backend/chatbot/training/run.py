...
Run this script to start the Nijenhuis Chatbot Training Framework as a standalone application
"""
...
def start_chatbot_server():
    ...
    try:
        ...
        if not os.path.exists("../api/server.py"):
            print("❌ server.py not found in ../api/ directory")
            return False
...
def run_training_framework():
    ...
    try:
        ...
        if not os.path.exists("framework.py"):
            print("❌ framework.py not found in current directory")
            return False
        
        # Import and run the training framework
        import framework as training_framework
...
def main():
    ...
    required_files = ["framework.py", "../api/server.py"]
    missing_files = []
