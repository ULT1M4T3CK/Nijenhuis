    ...
    class CartManager {
        ...
        async addItem(boatId, startDate, endDate, useMotor = false, quantity = 1) {
            ...
            try {
                ...
                console.warn('Availability check failed, proceeding with add:', error);
                // Continue if availability check fails (non-blocking)
            }
        ...
        if (cart.updateItem(itemId, item.startDate, item.endDate, quantity)) {
            ...
            try {
                ...
                if (response.ok) {
                    const data = await response.json();
                    if (data.success && data.data && data.data.availableCount !== undefined) {
                        const maxAvailable = data.data.availableCount;
