...
if (typeof CartManager !== 'undefined') {
    ...
    window.updateCartUI = function () {
        ...
        const adminPct = typeof window.CHECKOUT_ADMIN_FEE_PERCENT === 'number' ? window.CHECKOUT_ADMIN_FEE_PERCENT : 0;
        const adminFeeCart = Math.round(total * (adminPct / 100) * 100) / 100;
        const displayTotal = Math.round((total + adminFeeCart) * 100) / 100;
        ...
        if (content && footer && totalPrice) {
            if (count === 0) {
                ...
                totalPrice.textContent = `€${displayTotal.toFixed(2)}`;
